...
describe('Microtable', () => {
  ...
  beforeEach(() => {
    ...
    bigActivityRatings = Microtable.create(randomRecords(100000));
  });
  ...
  test('records returns matched records', () => {
    const records = activityRatings.where({person: 'Bob'}).records();
  ...
  test('get with a blank returns the right value', () => {
    const rating = activityRatings.get({
      person: 'Alice', activity: 'hiking', rating: blank
    });
    expect(rating).toEqual(5);
  })

  test('get with a blank returns undefined if there is no match', () => {
    const missingRating = activityRatings.get({
      person: 'Alice', activity: 'cycling', rating: blank
    });
    expect(missingRating).toBeUndefined();
  });

  test('get throws an error if there are multiple blanks', () => {
    expect(() => {
      activityRatings.get({
        person: 'Alice', activity: blank, rating: blank
      })
    }).toThrow();
  });

  test('get works with a single missing field', () => {
    const ratingWithImpliedBlank = activityRatings.get({
      person: 'Alice', activity: 'hiking', 
    });
    expect(ratingWithImpliedBlank).toEqual(5);
  });
    
  test('get throws an error if there are multiple missing fields', () => {
    expect(() => {
      activityRatings.get({
        person: 'Alice',
      })
  ...
  test('createIndex creates an index', () => {
    ...
    const resultWithoutIndex = bigActivityRatings.where({activity: 'hiking', rating: 5});
    ...
    const resultWithIndex = bigActivityRatings.where({activity: 'hiking', rating: 5});
    ...
    console.log({
      timeWithIndex, timeWithoutIndex
    })
  });
  ...
  test('insert adds a new record', () => {
    activityRatings.insert({person: 'Charlie', activity: 'hiking', rating: 4});
    const records = activityRatings.where({person: 'Charlie', activity: 'hiking'});
